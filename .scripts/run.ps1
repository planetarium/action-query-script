...
try {
    ...
    if (!$Quiet) {
        ...
        ./.scripts/write-json.ps1 -HashTable @{
    ...
    ./.scripts/write-title.ps1 "## Action Query`n" -Quiet:$Quiet
    $plainValue = ./.scripts/action-query.ps1 -Url $Url -Name $Name -Arguments $Arguments -Quiet:$Quiet

    ./.scripts/write-title.ps1 "## Nonce`n" -Quiet:$Quiet
    $nonce = ./.scripts/nonce.ps1 -Url $Url -Address $address -Quiet:$Quiet

    ./.scripts/write-title.ps1 "## Unsigned Transaction`n" -Quiet:$Quiet
    ...
    if (($confirmation -eq "yes") -or ($confirmation -eq "y")) {
        ./.scripts/write-title.ps1 "## Stage Transaction`n" -Quiet:$Quiet
        $transactionId = ./.scripts/transaction-stage.ps1 -Url $Url -SignedTransaction $signedTransaction -Quiet:$Quiet
        
        if ($Watch) {
            ./.scripts/write-title.ps1 "## Transaction Result`n" -Quiet:$Quiet
